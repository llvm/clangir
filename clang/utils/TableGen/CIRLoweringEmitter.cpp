...
void GenerateLowering(raw_ostream &OS, const Record *Operation) {
  ...
  std::string LLVMOpName = Operation->getValueAsString("LLVMOpName").str();
  ClassDefinitions +=
      "class CIR" + Name +
...
void clang::EmitCIRBuiltinsLowering(RecordKeeper &Records, raw_ostream &OS) {
  ...
  for (const auto *Builtin : Records.getAllDerivedDefinitions("LoweringInfo")) {
    if (Builtin->getValueAsBit("SimpleLowering"))
