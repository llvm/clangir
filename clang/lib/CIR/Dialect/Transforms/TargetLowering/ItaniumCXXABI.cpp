...
mlir::TypedAttr ItaniumCXXABI::lowerMethodConstant(
  ...
  // TODO(cir): Port virtual method pointer support to upstream.
  // Virtual method handling requires isVirtual() and getVtableOffset()
  // methods on MethodAttr which are not yet implemented in upstream.
...
// TODO(cir): Add this back when InsertMemberOp is ported to upstream.
// lowerMethodCast is unused until BaseMethodOp and DerivedMethodOp are ported.
// static mlir::Value lowerMethodCast(mlir::Operation *op, mlir::Value
// loweredSrc,
...
//   auto binOpKind = isDerivedToBase ? cir::BinOpKind::Sub :
//   cir::BinOpKind::Add; auto adjustedAdjField = cir::BinOp::create(
...
public:
  ...
  // TODO(cir): Add these back when VAArgOp and DeleteArrayOp lowering are
  // ported to upstream LoweringPrepareCXXABI.
