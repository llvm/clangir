...
static bool isAggregateType(AllocaOp allocaOp) {
  auto t = mlir::dyn_cast<mlir::cir::StructType>(allocaOp.getAllocaType());
  if (!t)
    return false;
...
void LifetimeCheckPass::checkAlloca(AllocaOp allocaOp) {
  ...
  case TypeCategory::Aggregate: {
    ...
    auto members =
        mlir::cast<mlir::cir::StructType>(allocaOp.getAllocaType()).getMembers();

    unsigned fieldIdx = 0;
    std::for_each(members.begin(), members.end(), [&](mlir::Type t) {
      auto ptrType = mlir::dyn_cast<mlir::cir::PointerType>(t);
      if (ptrType)
