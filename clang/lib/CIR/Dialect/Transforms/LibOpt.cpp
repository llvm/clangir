...
void LibOptPass::xformStdFindIntoMemchr(StdFindOp findOp) {
  ...
  auto containerTy = mlir::dyn_cast<StructType>(thisPtr);
  if (!containerTy)
    return;
  ...
  //   char size.
  auto iterResTy = mlir::dyn_cast<PointerType>(iterBegin.getResult().getType());
  assert(iterResTy && "expected pointer type for iterator");
  auto underlyingDataTy = mlir::dyn_cast<mlir::cir::IntType>(iterResTy.getPointee());
  ...
  //   - cast it to `int`.
  auto patternAddrTy = mlir::dyn_cast<PointerType>(findOp.getOperand(2).getType());
