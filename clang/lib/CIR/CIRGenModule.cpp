...
mlir::Value CIRGenModule::getAddrOfGlobalVar(const VarDecl *D,
  ...
  auto ptrTy = mlir::cir::PointerType::get(builder.getContext(), g.sym_type());
...
void CIRGenModule::buildGlobalVarDefinition(const clang::VarDecl *D,
  ...
  if (auto symAttr = Init.dyn_cast<mlir::SymbolRefAttr>()) {
    ...
    auto arrayTy = g.sym_type().dyn_cast<mlir::cir::ArrayType>();
...
generateStringLiteral(mlir::Location loc, mlir::Attribute C,
...
CIRGenModule::getAddrOfConstantStringFromLiteral(const StringLiteral *S,
  ...
  if (!getLangOpts().WritableStrings) {
    if (ConstantStringMap.count(C)) {
      ...
      if (!g.alignment() || uint64_t(Alignment.getQuantity()) > *g.alignment())
        g.alignmentAttr(getSize(Alignment));
  ...
  auto GV =
      generateStringLiteral(loc, C, LT, *this, GlobalVariableName, Alignment);
  ConstantStringMap[C] = GV;

  assert(!cir::UnimplementedFeature::reportGlobalToASan() && "NYI");
  return mlir::SymbolRefAttr::get(
      castStringLiteralToDefaultAddressSpace(*this, GV.sym_nameAttr()));
}
...
mlir::cir::FuncOp CIRGenModule::createCIRFunction(mlir::Location loc,
  ...
  FuncOp f;
  {
    ...
    f.linkageAttr(mlir::cir::GlobalLinkageKindAttr::get(
...
void CIRGenModule::applyReplacements() {
  for (auto &I : Replacements) {
    ...
    if (OldF.replaceAllSymbolUses(NewF.sym_nameAttr(), theModule).failed())
