...
public:
  ...
  virtual mlir::LogicalResult amendOperation(
    ...
    auto func = mlir::dyn_cast<mlir::LLVM::LLVMFuncOp>(op);
    if (!func)
      return mlir::success();
    llvm::Function *llvmFunc = moduleTranslation.lookupFunction(func.getName());
    if (auto extraAttr = mlir::dyn_cast<mlir::cir::ExtraFuncAttributesAttr>(attribute.getValue())) {
      for (auto attr : extraAttr.getElements()) {
        if (auto inlineAttr = mlir::dyn_cast<mlir::cir::InlineAttr>(attr.getValue())) {
