...
public:
  ...
  matchAndRewrite(mlir::cir::ConstantOp op, OpAdaptor adaptor,
    ...
    // then memcopyied into the stack (as done in Clang).
      ...
      rewriter.replaceOp(op, initVal);
      return mlir::success();
    } else if (mlir::isa<mlir::cir::StructType>(op.getType())) {
      auto attr = op.getValue();
      if (mlir::isa<mlir::cir::ZeroAttr, mlir::cir::UndefAttr>(attr)) {
        auto initVal =
            direct::lowerCirAttrAsValue(op, attr, rewriter, typeConverter);
        rewriter.replaceOp(op, initVal);
        return mlir::success();
      }
...
public:
  ...
  matchAndRewrite(mlir::cir::ShiftOp op, OpAdaptor adaptor,
    ...
    if (cirAmtTy)
      amt = getLLVMIntCast(rewriter, amt,
                           llvm::cast<mlir::IntegerType>(llvmTy),
