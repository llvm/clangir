...
public:
  ...
  matchAndRewrite(mlir::cir::PtrStrideOp ptrStrideOp, OpAdaptor adaptor,
    ...
    auto width = llvm::cast<mlir::IntegerType>(index.getType()).getWidth();
...
public:
  ...
  matchAndRewrite(mlir::cir::CastOp castOp, OpAdaptor adaptor,
    ...
    case mlir::cir::CastKind::int_to_float: {
      ...
      auto llvmSrcVal = adaptor.getOperands().front();
      auto llvmDstTy = getTypeConverter()->convertType(dstTy);
      if (mlir::cast<mlir::cir::IntType>(elementTypeIfVector(castOp.getSrc().getType()))
              .isSigned())
    ...
    case mlir::cir::CastKind::float_to_int: {
      ...
      auto llvmSrcVal = adaptor.getOperands().front();
      auto llvmDstTy = getTypeConverter()->convertType(dstTy);
      if (mlir::cast<mlir::cir::IntType>(elementTypeIfVector(castOp.getResult().getType()))
              .isSigned())
...
public:
  ...
  matchAndRewrite(mlir::cir::ConstantOp op, OpAdaptor adaptor,
    ...
    if (mlir::isa<mlir::cir::BoolType>(op.getType())) {
      ...
      attr = op.getValue();
    } else if (llvm::isa<mlir::cir::DataMemberType>(op.getType())) {
      auto dataMember = llvm::cast<mlir::cir::DataMemberAttr>(op.getValue());
    ...
    // then memcopyied into the stack (as done in Clang).
      ...
      if (!constArr &&
          !mlir::isa<mlir::cir::ZeroAttr, mlir::cir::UndefAttr>(op.getValue()))
      ...
      if (constArr && hasTrailingZeros(constArr)) {
        ...
        auto initVal =
            direct::lowerCirAttrAsValue(op, op.getValue(), rewriter, typeConverter);
      ...
      if (mlir::isa<mlir::cir::ZeroAttr, mlir::cir::UndefAttr>(attr)) {
        auto initVal = direct::lowerCirAttrAsValue(op, attr, rewriter, typeConverter);
        rewriter.replaceAllUsesWith(op, initVal);
        rewriter.eraseOp(op);
        return mlir::success();
      }
  ...
  matchAndRewrite(mlir::cir::VecShuffleOp op, OpAdaptor adaptor,
    ...
    std::transform(
        ...
        std::back_inserter(indices), [](mlir::Attribute intAttr) {
          return mlir::cast<mlir::cir::IntAttr>(intAttr).getValue().getSExtValue();
        });
  ...
  matchAndRewrite(mlir::cir::SwitchFlatOp op, OpAdaptor adaptor,
    ...
    if (op.getCaseValues()) {
      for (auto val : op.getCaseValues()) {
        auto intAttr = mlir::dyn_cast<mlir::cir::IntAttr>(val);
...
static mlir::Value createLLVMBitOp(mlir::Location loc,
  ...
  return getLLVMIntCast(
      rewriter, op->getResult(0), llvm::cast<mlir::IntegerType>(resultTy),
...
public:
  ...
  matchAndRewrite(mlir::cir::AtomicFetch op, OpAdaptor adaptor,
    ...
    if (auto intTy = mlir::dyn_cast<mlir::cir::IntType>(op.getVal().getType())) {
      isInt = true;
      isSignedInt = intTy.isSigned();
    } else if (mlir::isa<mlir::cir::SingleType, mlir::cir::DoubleType>(op.getVal().getType()))
      isInt = false;
    else {
      return op.emitError()
...
public:
  ...
  matchAndRewrite(mlir::cir::SetBitfieldOp op, OpAdaptor adaptor,
    ...
    resultVal =
        createIntCast(rewriter, resultVal, mlir::cast<mlir::IntegerType>(resultTy));
...
public:
  ...
  matchAndRewrite(mlir::cir::GetBitfieldOp op, OpAdaptor adaptor,
    ...
    auto resTy = getTypeConverter()->convertType(op.getType());
    auto newOp = createIntCast(rewriter, val, mlir::cast<mlir::IntegerType>(resTy),
                               info.getIsSigned());
    rewriter.replaceOp(op, newOp);
    return mlir::success();
  }
...
void prepareTypeConverter(mlir::LLVMTypeConverter &converter,
  ...
  converter.addConversion([&](mlir::cir::VectorType type) -> mlir::Type {
    auto ty = converter.convertType(type.getEltType());
    return mlir::LLVM::getVectorType(ty, type.getSize(), /*isScalable=*/false);
  });
