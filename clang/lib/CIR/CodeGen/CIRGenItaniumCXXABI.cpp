...
void CIRGenItaniumCXXABI::buildThrow(CIRGenFunction &CGF,
  ...
  auto typeInfo = mlir::dyn_cast_or_null<mlir::cir::GlobalViewAttr>(
...
mlir::Value CIRGenItaniumCXXABI::buildDynamicCastCall(
  ...
  mlir::Value srcRtti = CGF.getBuilder().getConstant(
      Loc,
      mlir::cast<mlir::TypedAttr>(CGF.CGM.getAddrOfRTTIDescriptor(Loc, SrcRecordTy.getUnqualifiedType())));
  mlir::Value destRtti = CGF.getBuilder().getConstant(
      Loc,
      mlir::cast<mlir::TypedAttr>(CGF.CGM.getAddrOfRTTIDescriptor(Loc, DestRecordTy.getUnqualifiedType())));
