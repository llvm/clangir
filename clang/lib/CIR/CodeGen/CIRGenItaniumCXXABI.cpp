...
void CIRGenItaniumCXXABI::buildGuardedInit(CIRGenFunction &cgf,
  ...
  // TODO(CIR): Map this to the globalOp as it needs to be consumed during
  // lowering prepare. This is hardcoded to `true` in
  // `LoweringPreparePass::handleStaticLocal` and must be fixed there at the
  // same time.
  assert(performInit && "NYI");
  ...
  cir::GlobalOp guard = CGM.getStaticLocalDeclGuardAddress(&varDecl);
  if (!guard) {
