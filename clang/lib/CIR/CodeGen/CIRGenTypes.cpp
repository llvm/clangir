...
mlir::Type CIRGenTypes::convertRecordDeclType(const clang::RecordDecl *RD) {
  ...
  const auto *key = Context.getCanonicalTagType(RD).getTypePtr();
  ...
  if (const auto *cxxRecordDecl = dyn_cast<CXXRecordDecl>(RD)) {
    for (const auto &I : cxxRecordDecl->bases()) {
      if (I.isVirtual())
        continue;
      convertRecordDeclType(I.getType()->castAs<RecordType>()->getOriginalDecl());
    }
  }
...
bool CIRGenTypes::isFuncParamTypeConvertible(clang::QualType Ty) {
  ...
  return isSafeToConvert(RT->getOriginalDecl(), *this);
}
...
#include "clang/AST/BuiltinTypes.def"
  ...
  case Type::ConstantArray: {
    ...
    auto isSized = [&](mlir::Type ty) {
      if (ty.isIntOrFloat() ||
          llvm::isa<mlir::cir::PointerType, mlir::cir::StructType,
                 mlir::cir::ArrayType, mlir::cir::BoolType>(ty))
        return true;
      assert(0 && "not implemented");
      return false;
    };
...
void CIRGenTypes::UpdateCompletedType(const TagDecl *TD) {
  ...
  if (const auto *ED = dyn_cast<EnumDecl>(TD)) {
    ...
    if (TypeCache.count(Context.getCanonicalTagType(ED).getTypePtr())) {
  ...
  if (recordDeclTypes.count(Context.getCanonicalTagType(RD).getTypePtr()))
