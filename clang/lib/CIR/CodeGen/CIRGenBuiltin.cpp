...
/// Build call arguments for an intrinsic call. This is a helper function
/// for buildIntrinsicCall.
void CIRGenFunction::buildIntrinsicCallArgs(
    CallArgList &args,
  ...
  // No support for right to left argument evaluation.
  assert(!CGM.getTarget().getCXXABI().areArgsDestroyedLeftToRightInCallee() &&
         "MSABI NYI");
  // Evaluate each argument.
  for (unsigned idx = 0; idx < argTypes.size(); ++idx)
    buildCallArg(args, *(argRange.begin() + idx), argTypes[idx]);
}

mlir::Value CIRGenFunction::buildIntrinsicCall(const CallExpr *e,
                                               ReturnValueSlot returnValue,
  ...
  cir::CallArgList callArgs;
  buildIntrinsicCallArgs(callArgs, e->arguments());

  clang::QualType calleeType = e->getCallee()->getType();
  auto pointeeType = cast<clang::PointerType>(calleeType)->getPointeeType();
  const auto *fnType = cast<clang::FunctionType>(pointeeType);
  const CIRGenFunctionInfo &callInfo = CGM.getTypes().arrangeFreeFunctionCall(
      callArgs, fnType, /*ChainCall=*/false);
  ...
  cir::ClangToCIRArgMapping cirFunctionArgs(CGM.getASTContext(), callInfo);
  ...
  for (CallArgList::const_iterator iter = callArgs.begin(),
                                   end = callArgs.end();
       iter != end; ++iter, ++info_it, ++argNo) {
    ...
    case ABIArgInfo::Direct: {
      if (!mlir::isa<mlir::cir::StructType>(argInfo.getCoerceToType()) &&
        ...
        assert(numCIRArgs == 1);
        mlir::Value V;
        assert(!iter->isAggregate() && "Aggregate NYI");
        V = iter->getKnownRValue().getScalarVal();
      ...
      if (!iter->isAggregate()) {
        ...
        src = iter->hasLValue() ? iter->getKnownLValue().getAddress()
                                : iter->getKnownRValue().getAggregateAddress();
      }
      ...
      if (sTy && argInfo.isDirect() && argInfo.getCanBeFlattened()) {
        ...
        if (srcTy != sTy)
          ...
          src = builder.createElementBitCast(loc, src, sTy);
        }
        ...
        cirCallArgs[firstCIRArg] = builder.createLoad(loc, src);
      } else {
        llvm_unreachable("NYI");
      }
  ...
  mlir::StringAttr nameAttr = builder.getStringAttr(intrinsicName);
  mlir::Operation *intrinsicCallOp = builder.create<mlir::cir::IntrinsicCallOp>(
      loc, nameAttr, retCIRTy, cirCallArgs);
  assert(intrinsicCallOp->getNumResults() == 1 && "NYI");
  return intrinsicCallOp->getResult(0);
}
