...
RValue CIRGenFunction::buildBuiltinExpr(const GlobalDecl GD, unsigned BuiltinID,
  ...
  case Builtin::BI__builtin_object_size: {
    ...
    auto ResType = mlir::dyn_cast<mlir::cir::IntType>(ConvertType(E->getType()));
  ...
  case Builtin::BI__builtin_mul_overflow: {
    ...
    auto ResultCIRTy =
        llvm::cast<mlir::cir::IntType>(CGM.getTypes().ConvertType(ResultQTy));
  ...
  if (auto V = buildTargetBuiltinExpr(BuiltinID, E, ReturnValue)) {
    switch (EvalKind) {
    case TEK_Scalar:
      if (llvm::isa<mlir::cir::VoidType>(V.getType()))
        return RValue::get(nullptr);
      return RValue::get(V);
    case TEK_Aggregate:
      llvm_unreachable("NYI");
...
mlir::cir::FuncOp CIRGenModule::getBuiltinLibFunction(const FunctionDecl *FD,
  ...
  if (FD->hasAttr<AsmLabelAttr>())
    ...
    if (getTriple().isPPC64() &&
      ...
      Name = StringRef(astCtx.BuiltinInfo.getName(BuiltinID).data(), 10);
  }
