...
public:
  ...
  mlir::Value buildScalarPrePostIncDec(const UnaryOperator *E, LValue LV,
    ...
    if (isInc && type->isBooleanType()) {
      ...
      if (llvm::isa<mlir::cir::SingleType, mlir::cir::DoubleType>(value.getType())) {
        ...
        if (llvm::isa<mlir::cir::LongDoubleType>(value.getType()))
          FS = &CGF.getTarget().getLongDoubleFormat();
        else if (llvm::isa<mlir::cir::FP16Type>(value.getType()))
          FS = &CGF.getTarget().getHalfFormat();
        else if (llvm::isa<mlir::cir::BF16Type>(value.getType()))
...
#undef HANDLEBINOP
  ...
  buildScalarConversion(mlir::Value Src, QualType SrcType, QualType DstType,
    ...
    if (auto DstPT = llvm::dyn_cast<mlir::cir::PointerType>(DstTy)) {
      llvm_unreachable("NYI");
    }

    if (llvm::isa<mlir::cir::PointerType>(SrcTy)) {
      // Must be an ptr to int cast.
      assert(llvm::isa<mlir::cir::IntType>(DstTy) && "not ptr->int?");
