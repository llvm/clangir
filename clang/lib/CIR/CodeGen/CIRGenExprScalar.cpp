...
public:
  ...
  mlir::Value VisitShuffleVectorExpr(ShuffleVectorExpr *E) {
    if (E->getNumSubExprs() == 2) {
      ...
      for (unsigned i = 2; i < E->getNumSubExprs(); ++i) {
        Indices.push_back(cir::IntAttr::get(
  ...
  mlir::Value buildScalarPrePostIncDec(const UnaryOperator *E, LValue LV,
    ...
    if (isInc && type->isBooleanType()) {
      value = Builder.create<cir::ConstantOp>(
      ...
      if (CGF.getContext().isPromotableIntegerType(type)) {
        ...
        auto promotedCirTy =
            mlir::dyn_cast<cir::IntType>(ConvertType(type));
      ...
      if (CGF.SanOpts.hasOneOf(
        ...
        auto Kind = E->isIncrementOp() ? cir::UnaryOpKind::Inc
                                       : cir::UnaryOpKind::Dec;
      ...
      if (const VariableArrayType *vla =
        ...
        if (CGF.getLangOpts().isSignedOverflowDefined()) {
          value = builder.create<cir::PtrStrideOp>(loc, value.getType(),
                                                         value, amt);
        } else {
          value = builder.create<cir::PtrStrideOp>(loc, value.getType(),
                                                         value, amt);
      ...
      if (mlir::isa<cir::SingleType, cir::DoubleType>(
              value.getType())) {
        ...
        auto amt = Builder.getConstant(
            loc, cir::FPAttr::get(value.getType(), F));
  ...
  mlir::Value VisitPlus(const UnaryOperator *E,
    ...
    return buildUnaryOp(E, cir::UnaryOpKind::Plus, operand);
  }
  ...
  mlir::Value VisitMinus(const UnaryOperator *E, QualType PromotionType) {
    ...
    return buildUnaryOp(E, cir::UnaryOpKind::Minus, operand);
  }
  ...
  mlir::Value buildUnaryOp(const UnaryOperator *E, cir::UnaryOpKind kind,
                           mlir::Value input) {
...
#undef HANDLEBINOP
    ...
    if (const MemberPointerType *MPT = LHSTy->getAs<MemberPointerType>()) {
      ...
      if (LHSTy->isVectorType()) {
        if (!E->getType()->isVectorType()) {
          ...
          return Builder.create<cir::VecCmpOp>(
  ...
  mlir::Value buildFloatToBoolConversion(mlir::Value src, mlir::Location loc) {
    ...
    return Builder.create<cir::CastOp>(
        loc, boolTy, cir::CastKind::float_to_bool, src);
  }
  ...
  mlir::Value buildIntToBoolConversion(mlir::Value srcVal, mlir::Location loc) {
    ...
    return Builder.create<cir::CastOp>(
        loc, boolTy, cir::CastKind::int_to_bool, srcVal);
  }
  ...
  buildScalarConversion(mlir::Value Src, QualType SrcType, QualType DstType,
      ...
      if (mlir::isa<cir::CIRFPTypeInterface>(DstTy)) {
        ...
        if (CGF.getContext().getTargetInfo().useFP16ConversionIntrinsics()) {
          ...
          Src = Builder.createCast(
              CGF.getLoc(Loc), cir::CastKind::floating, Src, CGF.FloatTy);
        }
    ...
    if (DstType->isHalfType() &&
      ...
      if (mlir::isa<cir::CIRFPTypeInterface>(SrcTy)) {
        ...
        return Builder.createCast(CGF.getLoc(Loc),
                                  cir::CastKind::floating, Src, DstTy);
      }
      DstTy = CGF.FloatTy;
    }
    ...
    if (DstTy != ResTy) {
      if (CGF.getContext().getTargetInfo().useFP16ConversionIntrinsics()) {
        ...
        Res = Builder.createCast(CGF.getLoc(Loc), cir::CastKind::floating,
                                 Res, ResTy);
      }
    }
...
mlir::Value ScalarExprEmitter::buildMul(const BinOpInfo &Ops) {
  ...
  return Builder.create<cir::BinOp>(
...
mlir::Value ScalarExprEmitter::buildSub(const BinOpInfo &Ops) {
  ...
  return Builder.create<cir::PtrDiffOp>(CGF.getLoc(Ops.Loc),
...
mlir::Value ScalarExprEmitter::VisitCastExpr(CastExpr *CE) {
  ...
  case CK_NullToMemberPointer: {
    ...
    if (MPT->isMemberFunctionPointerType()) {
      auto Ty = mlir::cast<cir::MethodType>(CGF.getCIRType(DestTy));
...
mlir::Value ScalarExprEmitter::VisitCallExpr(const CallExpr *E) {
  ...
  assert(!cir::MissingFeatures::buildLValueAlignmentAssumption());
  return V;
}
...
mlir::Value ScalarExprEmitter::VisitInitListExpr(InitListExpr *E) {
  ...
  if (E->getType()->isVectorType()) {
    ...
    auto VectorType =
        mlir::dyn_cast<cir::VectorType>(CGF.getCIRType(E->getType()));
        ...
        auto res = B.create<cir::TernaryOp>(
            ...
            [&](mlir::OpBuilder &B, mlir::Location Loc) {
              ...
              auto res = B.create<cir::ConstantOp>(
                  ...
                  Builder.getAttr<cir::BoolAttr>(Builder.getBoolTy(),
                                                       true));
...
mlir::Value CIRGenFunction::buildCheckedInBoundsGEP(
  ...
  mlir::Value GEPVal = builder.create<cir::PtrStrideOp>(
