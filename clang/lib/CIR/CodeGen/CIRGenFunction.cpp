...
void CIRGenFunction::buildKernelMetadata(const FunctionDecl *FD,
  ...
  if (const WorkGroupSizeHintAttr *A = FD->getAttr<WorkGroupSizeHintAttr>()) {
    workGroupSizeHintAttr = builder.getI32ArrayAttr({
        static_cast<int32_t>(A->getXDim()->EvaluateKnownConstInt(getContext()).getSExtValue()),
        static_cast<int32_t>(A->getYDim()->EvaluateKnownConstInt(getContext()).getSExtValue()),
        static_cast<int32_t>(A->getZDim()->EvaluateKnownConstInt(getContext()).getSExtValue()),
    });
  }
