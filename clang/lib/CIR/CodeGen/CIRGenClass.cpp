...
static void buildMemberInitializer(CIRGenFunction &CGF,
  ...
  QualType RecordTy = CGF.getContext().getCanonicalTagType(ClassDecl);
  LValue LHS;
...
public:
  ...
  void Emit(CIRGenFunction &CGF, Flags flags) override {
    ...
    QualType RecordTy = CGF.getContext().getTagType(ElaboratedTypeKeyword::None,
...
void CIRGenFunction::buildCXXAggrConstructorCall(
  ...
  if (constantCount) {
    auto constIntAttr = mlir::dyn_cast<mlir::cir::IntAttr>(constantCount.getValue());
  ...
  auto arrayTy = mlir::dyn_cast<mlir::cir::ArrayType>(arrayBase.getElementType());
