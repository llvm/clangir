...
static void buildAtomicCmpXchg(CIRGenFunction &CGF, AtomicExpr *E, bool IsWeak,
  ...
  builder.create<mlir::cir::IfOp>(
      loc, cmp, false, [&](mlir::OpBuilder &, mlir::Location) {
        auto ptrTy = mlir::cast<mlir::cir::PointerType>(Val1.getPointer().getType());
...
void CIRGenFunction::buildAtomicStore(RValue rvalue, LValue dest,
  ...
  if (!dest.isSimple()) {
    if (dest.isBitField())
      loc = dest.getBitFieldPointer().getLoc();
    else if (dest.isVectorElt())
      loc = dest.getVectorPointer().getLoc();
    else if (dest.isExtVectorElt())
      llvm_unreachable("NYI");
  }
