...
CIRGenFunction::buildCXXMemberPointerCallExpr(const CXXMemberCallExpr *E,
  ...
  const auto *RD =
      cast<CXXRecordDecl>(MPT->getQualifier().getAsType()->castAs<RecordType>()->getOriginalDecl());
...
static CIRUsualDeleteParams getUsualDeleteParams(const FunctionDecl *FD) {
  CIRUsualDeleteParams Params;
