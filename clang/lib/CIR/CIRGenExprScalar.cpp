...
#undef VISITCOMP
  ...
  mlir::Value buildConversionToBool(mlir::Value Src, QualType SrcType,
    ...
    assert((SrcType->isIntegerType() ||
            mlir::isa<::mlir::cir::PointerType>(Src.getType())) &&
           "Unknown scalar type to convert");

    assert(mlir::isa<mlir::IntegerType>(Src.getType()) &&
  ...
  buildScalarConversion(mlir::Value Src, QualType SrcType, QualType DstType,
    ...
    if (mlir::isa<::mlir::cir::PointerType>(DstTy)) {
      assert(0 && "not implemented");
    }

    if (mlir::isa<::mlir::cir::PointerType>(SrcTy)) {
      // Must be a ptr to int cast.
      assert(mlir::isa<mlir::IntegerType>(DstTy) && "not ptr->int?");
      assert(0 && "not implemented");
    }
