...
// CIR: #tbaa[[FLOAT:.*]] = #cir.tbaa_scalar_type_desc<id = "float">
// CIR: #tbaa[[LONG:.*]] = #cir.tbaa_scalar_type_desc<id = "long">
// CIR: #tbaa[[DOUBLE:.*]] = #cir.tbaa_scalar_type_desc<id = "double">
// CIR: #tbaa[[LONG_LONG:.*]] = #cir.tbaa_scalar_type_desc<id = "long long">
// CIR: #tbaa[[LONG_DOUBLE:.*]] = #cir.tbaa_scalar_type_desc<id = "long double">
...
void test_int_and_float(int *a, float *b) {
  ...
  // CIR: cir.scope
  // CIR: %[[TMP1:.*]] = cir.load deref %{{.*}} : !cir.ptr<!cir.ptr<!s32i>>, !cir.ptr<!s32i> tbaa([#tbaa[[PTR]]])
  // CIR: %[[TMP2:.*]] = cir.load %[[TMP1]] : !cir.ptr<!s32i>, !s32i tbaa([#tbaa[[INT]]])
  // CIR: cir.if
  // CIR: %[[C2:.*]] = cir.const #cir.fp<2
  // CIR: %[[TMP3:.*]] = cir.load deref %[[ARG_b:.*]] : !cir.ptr<!cir.ptr<!cir.float>>, !cir.ptr<!cir.float> tbaa([#tbaa[[PTR]]])
  // CIR: cir.store %[[C2]], %[[TMP3]] : !cir.float, !cir.ptr<!cir.float> tbaa([#tbaa[[FLOAT]]])
  // CIR: else
  // CIR: %[[C3:.*]] = cir.const #cir.fp<3
  // CIR: %[[TMP4:.*]] = cir.load deref %[[ARG_b]] : !cir.ptr<!cir.ptr<!cir.float>>, !cir.ptr<!cir.float> tbaa([#tbaa[[PTR]]])
...
void test_long_and_double(long *a, double *b) {
  ...
  // CIR: cir.scope
  // CIR: %[[TMP1:.*]] = cir.load deref %{{.*}} : !cir.ptr<!cir.ptr<!s64i>>, !cir.ptr<!s64i> tbaa([#tbaa[[PTR]]])
  // CIR: %[[TMP2:.*]] = cir.load %[[TMP1]] : !cir.ptr<!s64i>, !s64i tbaa([#tbaa[[LONG]]])
  // CIR: cir.if
  // CIR: %[[C2:.*]] = cir.const #cir.fp<2
  // CIR: %[[TMP3:.*]] = cir.load deref %[[ARG_b:.*]] : !cir.ptr<!cir.ptr<!cir.double>>, !cir.ptr<!cir.double> tbaa([#tbaa[[PTR]]])
  // CIR: cir.store %[[C2]], %[[TMP3]] : !cir.double, !cir.ptr<!cir.double> tbaa([#tbaa[[DOUBLE]]])
  // CIR: else
  // CIR: %[[C3:.*]] = cir.const #cir.fp<3
  // CIR: %[[TMP4:.*]] = cir.load deref %[[ARG_b]] : !cir.ptr<!cir.ptr<!cir.double>>, !cir.ptr<!cir.double> tbaa([#tbaa[[PTR]]])
...
void test_long_long_and_long_double(long long *a, long double *b) {
  ...
  // CIR: cir.if
  // CIR: %[[C2:.*]] = cir.const #cir.fp<2
  // CIR: %[[TMP3:.*]] = cir.load deref %[[ARG_b:.*]] : !cir.ptr<!cir.ptr<!cir.long_double<!cir.f80>>>, !cir.ptr<!cir.long_double<!cir.f80>> tbaa([#tbaa[[PTR]]])
  // CIR: cir.store %[[C2]], %[[TMP3]] : !cir.long_double<!cir.f80>, !cir.ptr<!cir.long_double<!cir.f80>> tbaa([#tbaa[[LONG_DOUBLE]]])
  // CIR: else
  // CIR: %[[C3:.*]] = cir.const #cir.fp<3
  // CIR: %[[TMP4:.*]] = cir.load deref %[[ARG_b]] : !cir.ptr<!cir.ptr<!cir.long_double<!cir.f80>>>, !cir.ptr<!cir.long_double<!cir.f80>> tbaa([#tbaa[[PTR]]])
...
void test_char(char *a, char* b) {
  ...
  // CIR: cir.scope
  // CIR: %[[TMP1:.*]] = cir.load deref %{{.*}} : !cir.ptr<!cir.ptr<!s8i>>, !cir.ptr<!s8i> tbaa([#tbaa[[PTR]]])
  ...
  // CIR: %[[TMP3:.*]] = cir.load deref %[[ARG_b:.*]] : !cir.ptr<!cir.ptr<!s8i>>, !cir.ptr<!s8i> tbaa([#tbaa[[PTR]]])
  ...
  // CIR: %[[TMP4:.*]] = cir.load deref %[[ARG_b]] : !cir.ptr<!cir.ptr<!s8i>>, !cir.ptr<!s8i> tbaa([#tbaa[[PTR]]])
...
// LLVM: ![[TBAA_LONG_LONG]] = !{![[TBAA_LONG_LONG_PARENT:.*]], ![[TBAA_LONG_LONG_PARENT]], i64 0}
// LLVM: ![[TBAA_LONG_LONG_PARENT]] = !{!"long long", ![[CHAR]], i64 0}
