...
auto base_to_derived(int Base2::*ptr) -> int Derived::* {
  return ptr;
  // CIR: %{{.+}} = cir.derived_data_member(%{{.+}} : !cir.data_member<!s32i in !ty_Base2>) [4] -> !cir.data_member<!s32i in !ty_Derived>
...
auto derived_to_base(int Derived::*ptr) -> int Base2::* {
  ...
  // CIR: %{{.+}} = cir.base_data_member(%{{.+}} : !cir.data_member<!s32i in !ty_Derived>) [4] -> !cir.data_member<!s32i in !ty_Base2>
...
auto base_to_derived_zero_offset(int Base1::*ptr) -> int Derived::* {
  return ptr;
  // CIR: %{{.+}} = cir.derived_data_member(%{{.+}} : !cir.data_member<!s32i in !ty_Base1>) [0] -> !cir.data_member<!s32i in !ty_Derived>
...
auto derived_to_base_zero_offset(int Derived::*ptr) -> int Base1::* {
  ...
  // CIR: %{{.+}} = cir.base_data_member(%{{.+}} : !cir.data_member<!s32i in !ty_Derived>) [0] -> !cir.data_member<!s32i in !ty_Base1>
