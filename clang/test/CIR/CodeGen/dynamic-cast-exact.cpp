...
Derived *ptr_cast(Base1 *ptr) {
  ...
  //      CHECK: %[[#SRC:]] = cir.load %{{.+}} : !cir.ptr<!cir.ptr<!ty_Base1>>, !cir.ptr<!ty_Base1>
  ...
  // CHECK-NEXT: %[[#SRC_VPTR_PTR:]] = cir.cast(bitcast, %[[#SRC]] : !cir.ptr<!ty_Base1>), !cir.ptr<!cir.ptr<!cir.ptr<!cir.func<!u32i ()>>>>
  ...
  // CHECK-NEXT:   %[[#RES:]] = cir.cast(bitcast, %[[#SRC]] : !cir.ptr<!ty_Base1>), !cir.ptr<!ty_Derived>
...
Derived &ref_cast(Base1 &ref) {
  ...
  // CHECK-NEXT: }
  // CHECK-NEXT: %{{.+}} = cir.cast(bitcast, %[[#SRC]] : !cir.ptr<!ty_Base1>), !cir.ptr<!ty_Derived>
}
...
Derived *ptr_cast_always_fail(Base2 *ptr) {
  return dynamic_cast<Derived *>(ptr);
  //      CHECK: %{{.+}} = cir.load %{{.+}} : !cir.ptr<!cir.ptr<!ty_Base2>>, !cir.ptr<!ty_Base2>
