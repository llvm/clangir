...
__m128 test_cmpnleps(__m128 A, __m128 B) {
  ...
  // CIR-NEXT: [[CAST:%.*]] = cir.cast(bitcast, [[CMP:%.*]] : !cir.vector<!s32i x 4>), !cir.vector<!cir.float x 4>
...
__m128d test_cmpnlepd(__m128d A, __m128d B) {
  ...
  // CIR-NEXT: [[CAST:%.*]] = cir.cast(bitcast, [[CMP]] :  !cir.vector<!s64i x 2>), !cir.vector<!cir.double x 2>
