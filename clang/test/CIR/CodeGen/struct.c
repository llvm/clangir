...
// CHECK-DAG: cir.func @shouldCopyStructAsCallArg
  ...
  // CHECK-DAG: %[[#LV:]] = cir.load %{{.+}} : !cir.ptr<!ty_S1>, !ty_S1
  // CHECK-DAG: cir.call @shouldCopyStructAsCallArg(%[[#LV]]) : (!ty_S1) -> ()
}
...
// CHECK-DAG: cir.scope {
// CHECK-DAG:   cir.alloca !ty_SLocal2E0, !cir.ptr<!ty_SLocal2E0>, ["loc", init] {alignment = 4 : i64}
