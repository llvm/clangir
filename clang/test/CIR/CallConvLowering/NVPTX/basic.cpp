...
// CHECK: cir.func dso_local @_Z5UCharh(%arg0: !u8i {cir.zeroext} loc({{.+}})) -> (!u8i {cir.zeroext})
unsigned char UChar(unsigned char c) {
  // CHECK: cir.call @_Z5UCharh(%{{.+}}) : (!u8i) -> !u8i
  return UChar(c);
}

// CHECK: cir.func dso_local @_Z6UShortt(%arg0: !u16i {cir.zeroext} loc({{.+}})) -> (!u16i {cir.zeroext})
...
// CHECK: cir.func dso_local @_Z5ULongm(%arg0: !u32i loc({{.+}})) -> !u32i
...
// CHECK: cir.func dso_local @_Z4Chara(%arg0: !s8i {cir.signext} loc({{.+}})) -> (!s8i {cir.signext})
char Char(signed char c) {
  // CHECK: cir.call @_Z4Chara(%{{.+}}) : (!s8i) -> !s8i
  return Char(c);
}

// CHECK: cir.func dso_local @_Z5Shorts(%arg0: !s16i {cir.signext} loc({{.+}})) -> (!s16i {cir.signext})
short Short(short s) {
...
// CHECK: cir.func dso_local @_Z4Longl(%arg0: !s32i loc({{.+}})) -> !s32i
long Long(long l) {
...
// CHECK: cir.func dso_local @_Z10StructFuncv() -> !rec_Struct
Struct StructFunc() {
  return { 0, 1, 2, 3, 4 };
}
