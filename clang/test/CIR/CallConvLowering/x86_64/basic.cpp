...
// CHECK: %[[#V0:]] = cir.alloca !ty_S1, !cir.ptr<!ty_S1>
// CHECK: %[[#V1:]] = cir.cast(bitcast, %arg0 : !u64i), !ty_S1
// CHECK: cir.store %[[#V1]], %[[#V0]] : !ty_S1, !cir.ptr<!ty_S1>
S1 s1(S1 arg) {

  /// Cast argument and result of the function call to the expected types.
  // CHECK: %[[#V9:]] = cir.cast(bitcast, %{{.+}} : !ty_S1), !u64i
  // CHECK: %[[#V10:]] = cir.call @_Z2s12S1(%[[#V9]]) : (!u64i) -> !u64i
  // CHECK: %[[#V11:]] = cir.cast(bitcast, %[[#V10]] : !u64i), !ty_S1
  s1({1, 2});

  // CHECK: %[[#V12:]] = cir.load %{{.+}} : !cir.ptr<!ty_S1>, !ty_S1
  // CHECK: %[[#V13:]] = cir.cast(bitcast, %[[#V12]] : !ty_S1), !u64i
