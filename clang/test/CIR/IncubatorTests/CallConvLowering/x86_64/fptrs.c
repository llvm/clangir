...
// CHECK:   %[[#V1:]] = cir.get_global @foo : !cir.ptr<!cir.func<(!u32i) -> !s32i>>
// CHECK:   %[[#V2:]] = cir.cast bitcast %[[#V1]] : !cir.ptr<!cir.func<(!u32i) -> !s32i>> -> !cir.ptr<!cir.func<(!rec_S) -> !s32i>>
// CHECK:   cir.store{{.*}} %[[#V2]], %[[#V0]] : !cir.ptr<!cir.func<(!rec_S) -> !s32i>>, !cir.ptr<!cir.ptr<!cir.func<(!rec_S) -> !s32i>>>
void bar() {
  myfptr a = foo;
}

// CHECK: cir.func {{.*@baz}}(%arg0: !u32i
// CHECK:   %[[#V0:]] = cir.alloca !rec_S, !cir.ptr<!rec_S>, [""] {alignment = 4 : i64}
// CHECK:   %[[#V1:]] = cir.cast bitcast %[[#V0]] : !cir.ptr<!rec_S> -> !cir.ptr<!u32i>
// CHECK:   cir.store{{.*}} %arg0, %[[#V1]] : !u32i, !cir.ptr<!u32i>
// CHECK:   %[[#V2:]] = cir.alloca !cir.ptr<!cir.func<(!rec_S) -> !s32i>>, !cir.ptr<!cir.ptr<!cir.func<(!rec_S) -> !s32i>>>, ["a", init]
// CHECK:   %[[#V3:]] = cir.get_global @foo : !cir.ptr<!cir.func<(!u32i) -> !s32i>>
// CHECK:   %[[#V4:]] = cir.cast bitcast %[[#V3]] : !cir.ptr<!cir.func<(!u32i) -> !s32i>> -> !cir.ptr<!cir.func<(!rec_S) -> !s32i>>
...
// CHECK:   %[[#V7:]] = cir.cast bitcast %[[#V0]] : !cir.ptr<!rec_S> -> !cir.ptr<!s32i>
// CHECK:   %[[#V8:]] = cir.load{{.*}} %[[#V7]] : !cir.ptr<!s32i>, !s32i
// CHECK:   %[[#V9:]] = cir.cast integral %[[#V8]] : !s32i -> !u32i
// CHECK:   %[[#V10:]] = cir.cast bitcast %[[#V5]] : !cir.ptr<!cir.func<(!rec_S) -> !s32i>> -> !cir.ptr<!cir.func<(!u32i) -> !s32i>>
// CHECK:   %[[#V11:]] = cir.call %[[#V10]](%[[#V9]]) : (!cir.ptr<!cir.func<(!u32i) -> !s32i>>, !u32i) -> !s32i
