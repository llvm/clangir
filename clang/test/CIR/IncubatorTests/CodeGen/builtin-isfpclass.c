...
void test_is_finite(__fp16 *H, float F, double D, long double LD) {
    ...
    // CHECK: cir.is_fp_class %{{.*}}, fcInf : (!cir.f16) -> !cir.bool

    res = __builtin_isinf(F);
    // CHECK: cir.is_fp_class %{{.*}}, fcInf : (!cir.float) -> !cir.bool

    res = __builtin_isinf(D);
    // CHECK: cir.is_fp_class %{{.*}}, fcInf : (!cir.double) -> !cir.bool

    res = __builtin_isinf(LD);
    // CHECK: cir.is_fp_class %{{.*}}, fcInf : (!cir.long_double<!cir.f80>) -> !cir.bool

    res = __builtin_isfinite(*H);
    // CHECK: cir.is_fp_class %{{.*}}, fcFinite : (!cir.f16) -> !cir.bool
    res = __builtin_isfinite(F);
    // CHECK: cir.is_fp_class %{{.*}}, fcFinite : (!cir.float) -> !cir.bool
    res = finite(D);
    // finite() is a user-declared function, not a builtin, so it's a call
    // CHECK: cir.call @finite

    res = __builtin_isnormal(*H);
    // CHECK: cir.is_fp_class %{{.*}}, fcNormal : (!cir.f16) -> !cir.bool
    res = __builtin_isnormal(F);
    // CHECK: cir.is_fp_class %{{.*}}, fcNormal : (!cir.float) -> !cir.bool

    res = __builtin_issubnormal(F);
    // CHECK: cir.is_fp_class %{{.*}}, fcSubnormal : (!cir.float) -> !cir.bool
    res = __builtin_iszero(F);
    // CHECK: cir.is_fp_class %{{.*}}, fcZero : (!cir.float) -> !cir.bool
    res = __builtin_issignaling(F);
    // CHECK: cir.is_fp_class %{{.*}}, fcSNan : (!cir.float) -> !cir.bool
}
...
// CHECK: cir.is_fp_class %{{.*}}, fcFinite : (!cir.float)
...
// CHECK: cir.is_fp_class %{{.*}}, fcNan : (!cir.float)
...
// CHECK: cir.is_fp_class %{{.*}}, fcSNan : (!cir.double)
...
// CHECK: cir.is_fp_class %{{.*}}, fcZero : (!cir.f16)
