...
SimplifyRetYieldBlocks<IfOp>::replaceScopeLikeOp(PatternRewriter &rewriter,
  ...
  if (checkAndRewriteRegion(ifOp.thenRegion(), rewriter).succeeded())
    regionChanged = true;
  if (checkAndRewriteRegion(ifOp.elseRegion(), rewriter).succeeded())
    regionChanged = true;
...
void MergeCleanupsPass::runOnOperation() {
  ...
  for (auto *o : opsToSimplify) {
    bool erase = false;
    (void)applyOpPatternsAndFold(o, frozenPatterns, &erase);
  }
}
} // namespace
