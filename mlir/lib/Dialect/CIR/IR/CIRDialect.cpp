...
LogicalResult ConstantOp::verify() {
  ...
  return checkConstantTypes(getOperation(), getType(), value());
}
...
LogicalResult GlobalOp::verify() {
  ...
  if (initial_value().hasValue()) {
    if (checkConstantTypes(getOperation(), sym_type(), *initial_value())
            .failed())
      return failure();
  }
...
LogicalResult cir::FuncOp::verify() {
  ...
  if (isExternal()) {
    if (linkage() != cir::GlobalLinkageKind::ExternalLinkage &&
        linkage() != cir::GlobalLinkageKind::ExternalWeakLinkage)
