...
static LogicalResult verify(IfOp op) { return success(); }
...
static LogicalResult verify(ScopeOp op) { return success(); }
  ...
  auto parseCase = [&]() -> ParseResult {
    ...
    if (parser.parseOptionalComma().failed() &&
      ...
      cases.push_back(cir::CaseAttr::get(
          mlir::IntegerAttr::get(intCondType, 0 /*placeholder*/), kindAttr,
          parser.getContext()));
    ...
    if (parser.parseInteger(val).failed())
      return ::mlir::failure();
    cases.push_back(cir::CaseAttr::get(mlir::IntegerAttr::get(intCondType, val),
...
void printSwitchOp(OpAsmPrinter &p, SwitchOp op,
  ...
  for (auto &r : regions) {
    ...
    // Case value
    p.printStrippedAttrOrType(attr.value());
...
static LogicalResult verify(SwitchOp op) { return success(); }
